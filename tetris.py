...
class Block(Rectangle):
    ...
    # permanent attributes of class Block.
    BLOCK_SIZE = 30
    OUTLINE_WIDTH = 3
...
class Shape():
    ...
    def get_lowest_block(self):
        '''returns the row of the lowest block
        in the shape.
        '''
        y_pos = [block.y for block in self.get_blocks()]
        return max(y_pos)
    ...
    def can_rotate(self, board):
        ...
        '''
        valid_move = True
        rotDir = self.get_rotation_dir()
        center = self.center_block
        for block in self.get_blocks():
            # rotate each block around the center block and
            # get new coordinates
            x = center.x - rotDir*center.y + rotDir*block.y
            y = center.y + rotDir*center.x - rotDir*block.x

            # check the can_move method of the board object
            # for each block
            if board.can_move(x, y):
                pass
            else:
                # if even one block can't move then
                # the shape cannot rotate
                valid_move = False
        return valid_move
...
class Board():
    ...
    def delete_row(self, y):
        ...
        '''
        # create the keys to look up in the board grid
        row = [(x, y) for x in range(0, self.width)]

        # iterate over the keys
        for grid_point in row:

            # undraw the block
            block = self.grid[grid_point]
            
            block.undraw()
            
            # delete the dictionary entry
            del self.grid[grid_point]
    ...
    def is_row_complete(self, y):        
        ...
        '''
        # create the dictionary keys for the row y
        row = [(x, y) for x in range(0, self.width)]

        # start checking for completeness
        row_complete = True
        for grid_point in row:
            # if the grid point has a block then it will
            # be a key of the grid dictionary
            if grid_point in self.grid.keys():
                pass
            else:
                # if any of the grid points are not in the
                # dictionary then the row is not complete
                row_complete = False
    ...
    def move_down_rows(self, y_start):
        ...
        # for each row above a removed row (inclusive)
        for row in range(y_start, -1, -1):
            ...
            for col in range(0, self.width):
                ...
                # check if the grid_point is occupied
                if grid_point in self.grid.keys():
                    ...
                    # new dictionary key
                    new_grid_point = (grid_point[0] + dx,
                                      grid_point[1] + dy)

                    # put the block in the new grid point
                    self.grid[new_grid_point] = block
    ...
    def game_over(self):
        ...
        msg = Text(Point(self.width * Block.BLOCK_SIZE//2,
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        random_shape = self.SHAPES[shape_index]
        y = 0
        x = self.BOARD_WIDTH//2
        return random_shape(Point(x,y))
    ...
    def do_move(self, direction):
        ...
        valid_move = self.current_shape.can_move(self.board, dx, dy)
        if valid_move:
    ...
    def piece_death(self):
        # save the shape blocks to the board grid
        self.board.add_shape(self.current_shape)

        # check if any rows are completed
        self.board.remove_complete_rows()

        # create a new shape at the top of the screen and draw it
        self.current_shape = self.create_new_shape()
        can_draw = self.board.draw_shape(self.current_shape)
    ...
    def do_rotate(self):
        ...
        valid_rotation = self.current_shape.can_rotate(self.board)
        if valid_rotation:
    ...
    def key_pressed(self, event):
        ...
        key = event.keysym
        if key == 'space':
            # find where the bottom of the board is
            dy = (self.BOARD_HEIGHT - 1) - \
                 self.current_shape.get_lowest_block()
            dx = 0
            valid_move = self.current_shape.can_move(self.board, dx, dy)
            if valid_move:
                # move the shape coordinates to the bottom of screen
                self.current_shape.move(dx,dy)
                self.piece_death()
                    
            elif not valid_move:
                # iterate until valid dy is found
                while not valid_move:
                    dy -= 1
                    valid_move = self.current_shape.can_move(self.board,
                                                             dx, dy)
                # move the shape coordinates to the bottom of screen
                self.current_shape.move(dx,dy)
